...
export default {
    ...
    cleanIncomingPiecesRook({isWhitePiece, shouldDeleteDeterminations, pieceBoxPosition, determinationPosition}) {
        ...
        const determinaitonPiece = $$$(determinationPieceBoxElement, chessConfig.chessPieceSelector)

        if(!determinaitonPiece) return

        const determinationPieceType = determinaitonPiece.getAttribute('piece-type')
