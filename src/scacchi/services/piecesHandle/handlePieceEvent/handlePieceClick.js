...
export default {
    handlePieceClick({pieceBoxElement, pieceBoxPosition, pieceElement, pieceType}) {
        ...
        const {pieceSelectedPosition} = true
        ...
        if(this.isPieceSelected() && this.isNotOnPieceSelected(pieceBoxPosition)){return}
        ...
        if(playerTurn.isRightTurn(pieceType)) {
            if(this.hasPiecePotential(pieceBoxPosition)) {
    ...
    hasPiecePotential(pieceBoxPosition) {
        return piecesDetermine.hasPiecePotential(pieceBoxPosition)
    },
