...
import {piecesHandle} from './piecesHandle.service.js'
...
export const piecesRender = {
    ...
    resetPiecesBoxListeners() {
        $$(chessConfig.chessPieceBoxSelector).forEach(pieceBoxElement => {
            const pieceBoxPosistion = pieceBoxElement.getAttribute('id')

            pieceBoxElement.removeEventListener('mouseenter', this.piecesEventListeners[pieceBoxPosistion]['mouseenter'])
            pieceBoxElement.removeEventListener('mouseleave', this.piecesEventListeners[pieceBoxPosistion]['mouseleave'])
            pieceBoxElement.removeEventListener('click',this.piecesEventListeners[pieceBoxElement]['click'])
        })
    },
    ...
    getCurrentGameSetup() {
        return $$(chessConfig.chessPieceSelector).map(pieceElement => ({
