...
export default{
    generatePotentialDeterminations({isWhitePiece, pieceBoxPosition, determinations}) {
        ...
        determinations.map(determinationFn => determinationFn(col, row)).filter(determinationPosition => {
            return this.filterPotentialDeterminations({isWhitePiece, determinationPosition})
        }).map(determinationPosition => {this.determinations[pieceBoxPosition][determinationPosition] = true})
    },
    ...
    filterPotentialDeterminations({isWhitePiece, determinationPosition}) {
        ...
        if(isWhitePiece && isWhitePieceDet || !isWhitePiece && isBlackPieceDet){
            return false
        }
        return true
